...
import static com.example.project_4.NYPizzaController.myOrder;
...
public class ChicagoPizzaController {
    ...
    public void AddToOrder(){
        NYPizza myPizza = null;
        Pizza myRealPizza;
        switch (flavor.getValue().toString()){
            case "Deluxe": myRealPizza = myPizza.createDeluxe();
            case "Meatzza": myRealPizza = myPizza.createMeatzza();
            case "BBQChicken": myRealPizza = myPizza.createBBQChicken();
            default: myRealPizza = myPizza.createBuildYourOwn();
        }
        ArrayList<Pizza> pizzas = new ArrayList<Pizza>();
        pizzas.add(myRealPizza);
        myOrder.add(pizzas);
    }
}
